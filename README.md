...
9. 회의를 통한 추가 화면 구현 및 데이터베이스 실제 구현(데이터 삽입, 수정, 삭제)
	...
	급식표 : [담당자 이시형] 급식 내용을 데이터베이스에 연동하였고 날짜 클릭 시 다이얼로그를 띄우는데 데이터베이스에 해당 날짜에 맞는 급식(데이터)이 없는 경우 급식이 없음을 보여주고 추가 등의 이벤트가 가능하고 급식이 있는 경우 데이터베이스에서 날짜에 맞는 급식(데이터)를 가져와 TexView로 보여주고 수정 등의 이벤트가 가능하도록 구현
